...
-(void) setupHeader
{
    ...
    if ([self respondsToSelector:@selector(headerText)])
    {
        ...
        if ([self respondsToSelector:@selector(headerByText)])
        {
            header.frame = CGRectMake(header.frame.origin.x, header.frame.origin.y, header.frame.size.width, header.frame.size.height + 60);
            
            label.frame = CGRectMake(label.frame.origin.x, 10, label.frame.size.width, label.frame.size.height - 15);
            [header addSubview:label];
            
            UILabel *ByText = [[UILabel alloc] initWithFrame:CGRectMake(header.frame.origin.x, label.frame.origin.y + label.frame.size.height, header.frame.size.width, 80)];
            ByText.text = LCL(self.headerByText);
            ByText.font = [UIFont fontWithName:@"HelveticaNeue-UltraLight" size:16];
            ByText.backgroundColor = [UIColor clearColor];
            if ([self respondsToSelector:@selector(tintColor)])
                ByText.textColor = self.tintColor;
            if ([self respondsToSelector:@selector(headerColor)])
                ByText.textColor = self.headerColor;
            
            ByText.textAlignment = NSTextAlignmentCenter;
            
            [header addSubview:ByText];
        }
        ...
        if ([self respondsToSelector:@selector(headerSubText)])
        {
            ...
            label.frame = CGRectMake(label.frame.origin.x, 10, label.frame.size.width, label.frame.size.height + 70);
            [header addSubview:label];
        ...
        /*if ([self respondsToSelector:@selector(headerMiniText)])
        {
            header.frame = CGRectMake(header.frame.origin.x, header.frame.origin.y, header.frame.size.width, header.frame.size.height + 60);
            ...
            UILabel *miniText = [[UILabel alloc] initWithFrame:CGRectMake(header.frame.origin.x, label.frame.origin.y + label.frame.size.height, header.frame.size.width, 20)];
            miniText.text = LCL(self.headerMiniText);
            miniText.font = [UIFont fontWithName:@"HelveticaNeue-UltraLight" size:30];
            miniText.backgroundColor = [UIColor clearColor];
            if ([self respondsToSelector:@selector(tintColor)])
                miniText.textColor = self.tintColor;
            if ([self respondsToSelector:@selector(headerColor)])
                miniText.textColor = self.headerColor;
            
             miniText.textAlignment = NSTextAlignmentCenter;
            
            [header addSubview:miniText];
            } */
       else
        {
